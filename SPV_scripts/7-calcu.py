...
def parse_option():
    ...
    parser.add_argument('--data_dir', default='results/SPV-2que-add-kdg-1.0-vrto0.8-sim1.5-5-3/gpt-3.5-turbo-0613', type=str)
    parser.add_argument('--_model', default='gpt-3.5-turbo-0613', type=str, help='gpt-3.5-turbo-0613')
    parser.add_argument('--baseline', default='SPV-calcu', type=str)
...
def main(args):
    ...
    for file in dataset_to_fileList[args.dataset]:
        ...
        for sample_id in range(len(samples)):
            ...
            sentence,label,target_position,target_word,pos_tag,gloss,eg_sent,prompt,answer_text,answer_add_text, soure_domain_list_text, soure_domain_score_text,  first_final_answer, second_final_answer = sample
            ground_truth_list.append(int(label))
            if second_final_answer == '':
                final_answer2 = random.sample(['exist', 'non-exist'], 1)
            else:
                final_answer2 = second_final_answer
            # compare the graph
            if first_final_answer != '' and final_answer2 == 'exist':
